...
export default function SearchComponent({ queryList, page }) {
  return (
    <div className='container mt-4 p-2 w-full h-auto search-component'>
      <ul className='ul-search w-full h-full grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-4 p-[10px]'>
        {queryList.map((queryItem, index) => {
          ...
          return (
            <li key={index} data-id={index+1} className='w-full h-[300px]'>
              <Link href={{ pathname: formattedLink, query: { og: encodeURIComponent(queryItem.link) } }} className='w-full h-[85%] block mb-[20px] text-[14px]'>
